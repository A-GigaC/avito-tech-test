...
func (r *prRepo) GetByID(ctx context.Context, prID string) (*model.PullRequestDB, error) {
	...
	err := r.db.WithContext(ctx).Where("pull_request_id = ?", prID).First(&pr).Error
	if err != nil {
		return nil, err
	}
	return &pr, nil
...
func (r *prRepo) GetByReviewerID(ctx context.Context, reviewerID string) ([]model.PullRequestDB, error) {
	...
	err := r.db.WithContext(ctx).Where("? = ANY(assigned_reviewers)", reviewerID).Find(&prs).Error
	if err != nil {
		return nil, err
	}
	return prs, nil
...
func (r *prRepo) Exists(ctx context.Context, prID string) (bool, error) {
	var count int64
	err := r.db.WithContext(ctx).Where("pr_id = ?", prID).Count(&count).Error
	if err != nil {
		return false, err
	}
	return count > 0, nil
